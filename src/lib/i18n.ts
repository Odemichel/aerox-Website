 ...
 */
export function mergeDicts(base: Record<string, string>, locale: Record<string, string>): Record<string, string> {
  return { ...base, ...locale };
}

export function getDict(lang: string): Record<string, string> {
  const l = (SUPPORTED_LOCALES as readonly string[]).includes(lang) ? (lang as Locale) : DEFAULT_LOCALE;
  return mergeDicts(dictionaries[DEFAULT_LOCALE], dictionaries[l]);
}

export function t(dict: Record<string, string>, key: string, vars?: Record<string, string | number>) {
  let s = dict[key] ?? key;
